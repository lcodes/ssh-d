 ...
 */
module libssh.callbacks;

public import libssh.libssh;
 ...
 * @param userdata Userdata to be passed to the callback function.
 */
alias ssh_status_callback = void function (ssh_session session, float status,
		void *userdata);
...
alias ssh_callbacks_struct *ssh_callbacks;
...
alias ssh_auth_pubkey_callback = int function (ssh_session session, const(char)* user, ssh_key_struct *pubkey,
		byte signature_state, void *userdata);
...
alias ssh_server_callbacks_struct *ssh_server_callbacks;
...
alias ssh_socket_callbacks_struct *ssh_socket_callbacks;
 ...
 * evolves with time.
 */
void ssh_callbacks_init(ssh_callbacks_struct* p) {
    p.size = (*p).sizeof;
}
 ...
 */
alias ssh_packet_callback = int function (ssh_session session, uint8_t type, ssh_buffer packet, void *user);
...
alias ssh_packet_callbacks_struct *ssh_packet_callbacks;
...
alias ssh_channel_callbacks_struct *ssh_channel_callbacks;
